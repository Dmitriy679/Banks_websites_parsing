...
# -*- coding: utf-8 -*-
"""
🔗 Единый пайплайн для скачивания PDF (ФИНАЛЬНАЯ ВЕРСИЯ):
...
4. Поддерживает фильтрацию по тексту в названии
"""
...
def download_pdf(url: str, save_dir: Path, display_name: str, prefix: str = "") -> bool:
    ...
    # 4. Санитизация и префикс
    filename = sanitize_filename(f"{prefix}{base_name}")

    # 🚫 Пропускаем файлы с ключевыми словами
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            for i in range(count):
                ...
                # ... (проверки на футер и навигацию остаются без изменений) ...
                try:
                ...
                # ✅ НОВЫЙ ФИЛЬТР: Исключаем PDF-файлы
...
def download_pdfs_from_page(
    ...
    for i, (pdf_url, display_name) in enumerate(pdf_data, 1):
        prefix = f"{i:02d}_"
        # 🔥 Передаём display_name в функцию скачивания
        if download_pdf(pdf_url, save_dir=save_dir, display_name=display_name, prefix=prefix):
            success += 1
        time.sleep(0.8)
