...
# Директория со скриптом — для корректного пути сохранения
...
def sanitize_filename(filename: str) -> str:
    ...
    return filename[:200]  # Ограничение длины
...
def download_pdf(url: str, save_dir: Path, prefix: str = "") -> bool:
    ...
    # 🚫 ПРОПУСКАЕМ файлы с ключевыми словами в названии
        ...
        try:
            ...
            for href in raw_links:
                if href:
                    ...
                    if full_url not in seen and 'psbank.ru' in full_url:
                        seen.add(full_url)
...
def run_pipeline(
    ...
    Путь сохранения: директория скрипта + output_root + структура папок
    """
    ...
    # 🔥 КЛЮЧЕВОЕ ИЗМЕНЕНИЕ: путь относительно скрипта
...
if __name__ == "__main__":
    ...
    OUTPUT_FOLDER = "psb_downloads"  # Относительный путь — будет привязан к скрипту
    HEADLESS = True
    MIN_DELAY = 1.0
    MAX_DELAY = 3.0
